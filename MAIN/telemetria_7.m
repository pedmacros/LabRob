...
subplot(2,1,1);
plot(tiempo,tel(:,2), tiempo,tel(:,3), tiempo,tel(:,4), tiempo,tel(:,5), tiempo,tel(:,6), tiempo,tel(:,7), tiempo,tel(:,8), tiempo,tel(:,9), tiempo,tel(:,10));
xlabel('Tiempo (ms)');
title('Sensores');
legend('Distancia Dch (cm)','Distancia Izq (cm)','Velocidad Izq (rad/s)','Velocidad Dch (rad/s)','Referencia (cm)','Referencia (rad/s)','x(m)','y(m)','Ángulo(rad)');
grid;

subplot(2,1,2);
plot(tiempo,tel(:,11), tiempo,tel(:,12), tiempo,tel(:,13));
title('Actuadores');
xlabel('Tiempo (ms)');
